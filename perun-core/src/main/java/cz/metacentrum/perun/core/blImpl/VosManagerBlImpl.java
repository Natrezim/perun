...
public class VosManagerBlImpl implements VosManagerBl {
	...
	public List<MemberCandidate> getCompleteCandidates(PerunSession sess, Vo vo, List<String> attrNames, String searchString) throws InternalErrorException, MemberNotExistsException, ExtSourceNotExistsException {
		...
		List<Set<MemberCandidate>> result = new ArrayList<>();
		...
		for (MemberCandidate outerMC : memberCandidates) {
			for (MemberCandidate innerMC : memberCandidates) {
				...
				if (compareUserExtSourceLists(outerMC.getUserExtSources(), innerMC.getUserExtSources())) {
					...
					Set<MemberCandidate> newSet = new HashSet<>();
					newSet.add(outerMC);
					result.add(newSet);
				}
			}
...
//		Set<Set<MemberCandidate>> toDelete = new HashSet<>();
//
//		for (Set<MemberCandidate> outer : result) {
//			for (Set<MemberCandidate> inner : result) {
...
//		}
	...
	private boolean compareUserExtSourceLists(List<UserExtSource> list1, List<UserExtSource> list2) {
		for (UserExtSource ues1 : list1) {
			for (UserExtSource ues2 : list2) {
				if (ues1.getLogin().equals(ues2.getLogin())) {
					return true;
				}
			}
		}
	...
	private void checkList(List<Set<MemberCandidate>> result, MemberCandidate outer, MemberCandidate inner) {
		for (Set<MemberCandidate> memberCandidateList : result) {
			if (memberCandidateList.contains(outer)) {
				memberCandidateList.add(inner);
				return;
			}
		}
